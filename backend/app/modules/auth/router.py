...
from app.modules.users.service import get_user_by_email, create_user, get_user_by_id, delete_user
from app.modules.auth.schemas import LoginRequest, Role, TokenResponse, RegisterRequest, RegisterResponse
    ...
    if not user.get("is_active", False):
        ...
        if approval == "PENDING_APPROVAL":
            raise HTTPException(
                ...
                detail="Your account is pending admin approval. You will be able to access your account once approved by an administrator.",
            )
        ...
        # If explicitly suspended (can be a flag or just inactive)
...
async def register(request: RegisterRequest) -> RegisterResponse:
    ...
    if request.role == Role.ENTERPRISE:
        ...
        if not request.company_name:
            # We already created the user, so if validation fails here, we should cleanup.
        ...
        try:
            ...
            logger.error("Failed to create organization during registration: %s", e)
            await delete_user(str(user["_id"]))
            raise HTTPException(status_code=500, detail="Registration failed during organization creation.")
