...
const CATEGORIES = [
    { value: 'Exhibition', key: "organizer.newEvent.categories.exhibition" },
    { value: 'Conference', key: "organizer.newEvent.categories.conference" },
    { value: 'Webinar', key: "organizer.newEvent.categories.webinar" },
    { value: 'Networking', key: "organizer.newEvent.categories.networking" },
    { value: 'Workshop', key: "organizer.newEvent.categories.workshop" },
    { value: 'Hackathon', key: "organizer.newEvent.categories.hackathon" },
];
...
export default function NewEventRequestPage() {
    ...
    const [form, setForm] = useState({
        ...
        location: t("organizer.newEvent.placeholders.category"),
        tags: '',
        banner_url: '',
        num_enterprises: '',
        extended_details: '',
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <select name="category" value={form.category} onChange={handleChange} className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                {CATEGORIES.map((cat) => (<option key={cat.value} value={cat.value}>{t(cat.key)}</option>))}
                            </select>
                        </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700">
                            {t("organizer.newEvent.sections.additionalInfo")} <span className="text-gray-400">{t("organizer.newEvent.sections.additionalInfoOptional")}</span>
                        </label>
                    ...
                    <div className="flex gap-3 pt-2">
                        <Button type="submit" isLoading={saving} className="bg-indigo-600 hover:bg-indigo-700">
                            {saving ? t("organizer.newEvent.buttons.submitting") : t("organizer.newEvent.buttons.submit")}
                        </Button>
                        <Link href="/organizer/events">
                            <Button type="button" variant="outline">{t("organizer.newEvent.buttons.back")}</Button>
                        </Link>
                    </div>
                </form>
            </Card>
