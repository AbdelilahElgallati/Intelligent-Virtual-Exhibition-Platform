...
const ChatItem = ({ room, active, onClick, unreadCount, t }: { room: ChatRoom; active: boolean; onClick: () => void; unreadCount?: number; t: TFunction }) => {
    return (
        <div
            onClick={onClick}
            className={clsx(
        ...
        >
            ...
            {room.type === 'direct' && <span title={t('enterprise.communications.chats.b2bChat')}><Users size={12} className="text-zinc-300" /></span>}
        </div>
    );
};
...
const MeetingItem = ({ meeting, timeZone, onStatusUpdate, currentTime, t }: { meeting: Meeting; timeZone: string; onStatusUpdate: (id: string, status: string) => void; currentTime: number; t: TFunction }) => {
    const statusStyles = {
