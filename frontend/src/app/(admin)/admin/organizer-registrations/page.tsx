...
export default function OrganizerRegistrationsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-2 flex-wrap">
                {STATUS_TABS.map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                        ...
                        {tab === 'ALL' ? t('admin.approvals.organizer.tabs.all') : t(`admin.approvals.organizer.tabs.${tab === 'PENDING_APPROVAL' ? 'pending' : tab === 'APPROVED' ? 'approved' : 'rejected'}`)}
                    </button>
                ))}
            ...
            {loading ? (
                <div className="flex items-center justify-center h-48 text-zinc-400">{t('admin.approvals.organizer.loading')}</div>
                                ...
                                <div className="space-y-1 flex-1 min-w-0">
                                    <div className="flex items-center gap-3 flex-wrap">
                                        ...
                                        <span className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${STATUS_COLORS[reg.approval_status] || 'bg-zinc-100 text-zinc-600 border-zinc-200'}`}>
                                            {t(`admin.approvals.organizer.status.${reg.approval_status.toLowerCase()}`)}
                                        </span>
                                    </div>
