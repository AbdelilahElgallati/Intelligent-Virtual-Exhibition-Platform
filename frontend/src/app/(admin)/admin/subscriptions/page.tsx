...
export default function AdminSubscriptionsPage() {
    ...
    const handleOverride = async (orgId: string, currentPlan: string, orgName: string) => {
        ...
        try {
            ...
            setError(e.message ?? t('admin.subscriptions.actionFailed'));
        } finally {
            setActionId(null);
        }
    };
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-zinc-100">
                                <th className="text-left px-6 py-3.5 text-xs font-semibold text-zinc-500 uppercase tracking-wide">{t('admin.subscriptions.table.organization')}</th>
                                ...
                                <th className="text-left px-4 py-3.5 text-xs font-semibold text-zinc-500 uppercase tracking-wide hidden md:table-cell">{t('admin.subscriptions.table.status')}</th>
                                <th className="px-6 py-3.5 text-right text-xs font-semibold text-zinc-500 uppercase tracking-wide">{t('admin.subscriptions.table.actions')}</th>
                            </tr>
                        </thead>
                    ...
                    <div className="px-6 py-4 border-t border-zinc-100 flex items-center justify-between bg-white">
                        <span className="text-xs text-zinc-500">
                            {t('common.ui.pagination.showingRange', {
                                ...
                                total: subs.length,
                                entity: t('common.ui.pagination.entities.subscriptions')
                            })}
                        </span>
                        {totalPages > 1 && (
