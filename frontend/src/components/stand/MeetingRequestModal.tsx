...
function getMeetingTimeline(
    m: Meeting,
    t: (key: string, options?: Record<string, unknown>) => string
): { status: TimelineStatus; label: string; color: string; bgColor: string; icon: typeof Clock } {
    const now = Date.now();
    const start = new Date(m.start_time).getTime();
    const end = new Date(m.end_time).getTime();
    const minsUntilStart = Math.round((start - now) / 60000);

    if (m.status === 'rejected') return { status: 'ended', label: t('visitor.meetingRequestModal.status.rejected'), color: 'text-red-600', bgColor: 'bg-red-50 border-red-100', icon: Ban };
    if (m.status === 'canceled') return { status: 'ended', label: t('visitor.meetingRequestModal.status.canceled'), color: 'text-zinc-500', bgColor: 'bg-zinc-50 border-zinc-200', icon: Ban };
    if (m.status === 'completed' || m.session_status === 'ended') return { status: 'ended', label: t('visitor.meetingRequestModal.status.completed'), color: 'text-zinc-500', bgColor: 'bg-zinc-50 border-zinc-200', icon: CheckCircle2 };
    if (m.session_status === 'live') return { status: 'live', label: t('visitor.meetingRequestModal.status.liveNow'), color: 'text-emerald-600', bgColor: 'bg-emerald-50 border-emerald-200', icon: CircleDot };
    if (now > end) return { status: 'expired', label: t('visitor.meetingRequestModal.status.expired'), color: 'text-red-500', bgColor: 'bg-red-50/50 border-red-100', icon: Timer };
    if (now >= start && now <= end) return { status: 'live', label: t('visitor.meetingRequestModal.status.readyToJoin'), color: 'text-emerald-600', bgColor: 'bg-emerald-50 border-emerald-200', icon: Video };
    if (minsUntilStart <= 15 && minsUntilStart > 0) return { status: 'starting-soon', label: t('visitor.meetingRequestModal.status.inMinutes', { n: minsUntilStart }), color: 'text-orange-600', bgColor: 'bg-orange-50 border-orange-200', icon: Hourglass };
    if (minsUntilStart <= 60) return { status: 'upcoming', label: t('visitor.meetingRequestModal.status.inMinutes', { n: minsUntilStart }), color: 'text-indigo-600', bgColor: 'bg-indigo-50 border-indigo-200', icon: Clock };
    const hoursUntil = Math.floor(minsUntilStart / 60);
    if (hoursUntil < 24) return { status: 'upcoming', label: t('visitor.meetingRequestModal.status.inHours', { n: hoursUntil }), color: 'text-indigo-600', bgColor: 'bg-indigo-50 border-indigo-200', icon: Clock };
    return { status: 'upcoming', label: t('visitor.meetingRequestModal.status.inDays', { n: Math.floor(hoursUntil / 24) }), color: 'text-indigo-600', bgColor: 'bg-indigo-50 border-indigo-200', icon: Clock };
}
...
export function MeetingRequestModal({
    ...
    const categorizedMeetings = filteredMeetings.map((m) => ({ ...m, timeline: getMeetingTimeline(m, t) }));
