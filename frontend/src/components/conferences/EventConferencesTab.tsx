...
function statusStyle(status: CardStatus, t: (key: string, options?: Record<string, unknown>) => string): { label: string; text: string; bg: string; border: string } {
    if (status === 'live') {
...
export default function EventConferencesTab({ eventId, event: initialEvent }: EventConferencesTabProps) {
    ...
    }, [meetings, getMeetingStatus, nowMs, t]);
    ...
    const conferenceCards = useMemo<ConferenceCardModel[]>(() => {
        return conferences
            ...
            .sort((a, b) => parseMs(a.startTime) - parseMs(b.startTime));
    }, [conferences, eventId, getConferenceStatus, nowMs, t]);
        ...
        <div style={{ fontFamily: 'Inter, sans-serif' }}>
            ...
            {!loading && (
                <div style={{ display: 'grid', gap: 26 }}>
                    ...
                    <section>
                        <div style={{ marginBottom: 10 }}>
                            <h3 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: '#0f172a' }}>{t('visitor.eventConferencesTab.conferencesTitle')}</h3>
                            <p style={{ margin: '4px 0 0 0', fontSize: 13, color: '#64748b' }}>
