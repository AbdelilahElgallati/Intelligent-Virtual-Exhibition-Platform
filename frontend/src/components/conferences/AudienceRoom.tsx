...
function formatDateTime(value: string | undefined, t: (key: string) => string) {
  if (!value) return t('visitor.audienceRoom.notSet');
...
export default function AudienceRoom({
  ...
  let sessionState = t('visitor.audienceRoom.liveSession');
  if (start && now < start) {
    sessionState = t('visitor.audienceRoom.startsIn', { time: formatDuration(start - now) });
  } else if (start && end && isInSession) {
    sessionState = t('visitor.audienceRoom.endsIn', { time: formatDuration(end - now) });
  } else if (start && end) {
    sessionState = t('visitor.audienceRoom.endedAgo', { time: formatDuration(now - end) });
  }
  ...
  if (activePanel === 'chat') {
    sidePanelContent = (
      <ConferenceChatPanel
        title={t('visitor.audienceRoom.audienceChat')}
    ...
    sidePanelContent = (
        <div className="p-8 text-center">
          <p className="text-xs text-zinc-600">{t('visitor.audienceRoom.qaDisabled')}</p>
        </div>
      );
  }
      ...
      <div className="grid h-full min-w-0 grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          <header className="border-b border-white/5 bg-[#0a0a0f] px-6 py-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-2 rounded-lg bg-red-500/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-red-500 border border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]">
                    ...
                    <span>{t('visitor.audienceRoom.liveEvent')}</span>
                  </span>
                  ...
                  {reconnecting && (
                    <span className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500/10 px-2.5 py-1.5 text-[10px] font-semibold text-amber-500 border border-amber-500/10">
                      ...
                      {t('visitor.audienceRoom.reconnecting')}
                    </span>
                  )}
                </div>
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-4 text-xs text-zinc-500">
                  <span className="inline-flex items-center gap-1.5 font-medium">
                    ...
                    <span className="text-zinc-300 font-bold">{totalWatching}</span> {t('visitor.audienceRoom.peopleWatching')}
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <CalendarClock size={14} className="text-zinc-600" />
                    {formatDateTime(startTime, t)} – {formatDateTime(endTime, t)}
                  </span>
                </div>
              </div>
              <button
              ...
              >
                {t('visitor.audienceRoom.leaveRoom')}
              </button>
            </div>
          </header>
          ...
          <main className="relative flex-1 overflow-hidden bg-black flex flex-col p-4">
            ...
            {isDisconnected && (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
                <div className="max-w-md rounded-2xl border border-white/10 bg-zinc-900 p-8 text-center text-zinc-100 shadow-2xl">
                  ...
                  <h3 className="text-lg font-bold">{t('visitor.audienceRoom.sessionDisconnected')}</h3>
                  <p className="mt-2 text-sm text-zinc-400">
                    {t('visitor.audienceRoom.disconnectedMessage')}
                  </p>
                  <button
                  ...
                  >
                    ...
                    {t('visitor.audienceRoom.reconnectStream')}
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="absolute bottom-10 left-10 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-black/60 backdrop-blur-xl border border-white/10 shadow-2xl pointer-events-none">
              ...
              <span className="text-xs font-bold text-white tracking-wide">
                {t('visitor.audienceRoom.stage')}: {speakerName || enterpriseName || t('visitor.audienceRoom.assignedSpeaker')}
              </span>
            </div>
            ...
            <div className="absolute bottom-8 right-8 z-20 flex items-center gap-3 rounded-2xl border border-white/10 bg-black/55 px-4 py-3 backdrop-blur-xl">
              <button
                ...
                title={micOn ? t('visitor.audienceRoom.muteMicrophone') : t('visitor.audienceRoom.unmuteMicrophone')}
              >
              ...
              <button
                ...
                title={camOn ? t('visitor.audienceRoom.turnOffCamera') : t('visitor.audienceRoom.turnOnCamera')}
              >
        ...
        <aside className="flex flex-col bg-[#0a0a0f] border-l border-white/5 shadow-2xl overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-white/5">
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">{t('visitor.audienceRoom.conferenceIntel')}</h2>
            <div className="mt-4 flex items-center gap-3">
              ...
              <div>
                  <p className="text-sm font-bold text-white leading-none">{t('visitor.audienceRoom.audiencePanel')}</p>
                  <p className="text-[10px] text-zinc-500 mt-1">{t('visitor.audienceRoom.realtimeInteraction')}</p>
                </div>
              </div>
          </div>
            ...
            <div className="p-6">
              <div className="rounded-2xl bg-white/5 border border-white/5 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{t('visitor.audienceRoom.speaker')}</p>
                  <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500 text-[9px] font-bold border border-emerald-500/20">{t('visitor.audienceRoom.active')}</span>
                </div>
                <p className="text-sm font-bold text-zinc-100">{speakerName || enterpriseName || t('visitor.audienceRoom.notSpecified')}</p>
                <p className="text-xs text-zinc-400 leading-relaxed italic line-clamp-3">
                  "{conferenceDescription || t('visitor.audienceRoom.defaultDescription')}"
                </p>
              </div>
              ...
              <div className="mt-8">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t('visitor.audienceRoom.liveAttendees')}</p>
                  <span className="text-[10px] text-zinc-400 bg-white/5 px-2 py-0.5 rounded-full border border-white/5">{t('visitor.audienceRoom.onlineCount', { n: totalWatching })}</span>
                </div>
                
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/5 border border-white/5">
                    <div className="w-2 h-2 rounded-full bg-indigo-500" />
                    <span className="text-xs font-semibold text-white">{t('visitor.audienceRoom.you')}</span>
                  </div>
                  ...
                  {remoteParticipants.length === 0 && (
                    <p className="text-[10px] text-zinc-600 text-center py-2">{t('visitor.audienceRoom.noOtherAttendees')}</p>
                  )}
                </div>
              </div>
