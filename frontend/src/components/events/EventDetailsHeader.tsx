...
export const EventDetailsHeader: React.FC<EventDetailsHeaderProps> = ({ event }) => {
  ...
  const statusConfig = {
    UPCOMING: { class: 'bg-cyan-100 text-cyan-700 border-cyan-200', label: t('events.detail.joinCard.badges.upcoming') },
    LIVE: { class: 'bg-emerald-100 text-emerald-700 border-emerald-200', label: t('events.detail.joinCard.badges.liveAccess') },
    IN_PROGRESS: { class: 'bg-blue-100 text-blue-700 border-blue-200', label: t('events.detail.joinCard.badges.inProgress') },
    ENDED: { class: 'bg-slate-100 text-slate-700 border-slate-200', label: t('events.detail.joinCard.badges.ended') },
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="flex-grow space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline">{event.category || t('visitor.eventCard.defaultCategory')}</Badge>
              {event.tags?.map((tag) => (
            ...
            <div className="flex flex-wrap gap-6 text-sm text-muted-foreground pt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{t('visitor.eventDetailsHeader.organizedBy', { name: event.organizer_name || 'IVEP Organizer' })}</span>
              </div>
            </div>
          </div>
        </div>
