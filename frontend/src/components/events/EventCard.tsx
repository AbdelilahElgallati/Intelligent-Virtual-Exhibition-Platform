...
export const EventCard: React.FC<EventCardProps> = ({ event }) => {
  ...
  const actionLabel = lifecycle.accessState === 'OPEN_SLOT_ACTIVE' ? t('visitor.eventCard.enterLiveEvent') : lifecycle.displayState === 'UPCOMING' ? t('visitor.eventCard.viewDetails') : t('visitor.eventCard.viewSummary');
      ...
      <CardContent className="p-6 flex-grow">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 text-sm text-zinc-600">
            ...
            <span className="font-medium truncate">{event.location || t('visitor.eventCard.virtualFallback')}</span>
          </div>
          <div className="flex items-center gap-3 text-sm text-zinc-600">
        ...
        <div className="w-full flex items-center justify-between">
          {(lifecycle.displayState === 'UPCOMING' || lifecycle.displayState === 'IN_PROGRESS') && lifecycle.nextSlot && (
            <div className="flex items-center gap-2 text-xs font-bold text-primary animate-in fade-in duration-500">
              <Clock size={12} />
              <span>{lifecycle.displayState === 'IN_PROGRESS' ? `${t('visitor.eventCard.nextSlot')} ` : ''}{formatTimeToStart(lifecycle.nextSlot.start)}</span>
            </div>
          )}
